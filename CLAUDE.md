...
- **Анонсы** — src/hooks/useAnnounceData.ts
  ID: 1X-11DKj8qvSFHlU4lacf0JHFDfR4qEN_zwmtSGwTr_E
  Формат: вертикальные блоки «ключ-значение» (col[1]=ключ, col[2]=значение).
  Ключи: название (открывает новый блок), Date, Elo limit, Mappool, Prize Pool, Brackets, инфо.
  Date показывается как есть; для отсчёта парсится "14th June 2026, 16:00 MSK" или "14.06.2026 16:00" — время указывать MSK первым.
  Ссылка CTA = первое http-значение блока, иначе Discord-инвайт. Прошедшие даты скрываются автоматически.
...
- Данные из таблицы анонсов (useAnnounceData, см. выше)
...
- Таблица: # | Игрок | ELO | ± (grid, сквозные границы колонок)
...
- ВАЖНО (sticky-архитектура): кнопки дивизионов + поиск + ЗАГОЛОВОК столбцов = ОДИН sticky-блок (top: 79 = нав 48 + саб-нав 32 − 1px нахлёст). Заголовок столбцов и верхняя грань рамки лежат ВНУТРИ этой плашки. Строки таблицы — отдельная секция ниже, без sticky, рамка только слева/справа/снизу.
  НЕ выносить заголовок в отдельный sticky с offset по измеренному panelH — это давало зазор, сквозь который при инерционном скролле вверх проступали строки (рассинхрон двух sticky). panelH (ResizeObserver) теперь нужен ТОЛЬКО для scrollMarginTop при прыжке к дивизиону, где неточность не видна.
...
- Саб-нав: фиксированная высота 32px — sticky-offset плашки завязан на неё (79px)
- Поле поиска: fontSize:16 обязательно — иначе мобильный браузер зумирует

## Фон
- Клетка-грид — отдельный статичный fixed-слой `body::before` (position:fixed, inset:0, z-index:-1; --grid-rgb/--grid-line по теме). На body — только background-color.
- ВАЖНО: НЕ ставить background-image+`background-attachment:fixed` на body — это уводит браузер на медленный путь скролла и рвёт перекрывающие sticky-элементы (мерцание/зазор при инерционном скролле). Фиксированный слой ::before статичен и не перерисовывается при скролле.
- JS-параллакс фона убран (useGridParallax не используется) — давал дёрганый скролл.
- Сканлайнов нет (убраны совсем); компонент ScanLine в UI.tsx больше нигде не рендерится
- App-обёртка фон не задаёт — иначе перекроет грид
