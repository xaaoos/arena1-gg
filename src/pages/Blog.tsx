    ...
    <div style={{ overflowX: "hidden" }}>
      ...
      <section style={{ minHeight: "100vh", padding: mob ? "80px 16px 40px" : "120px 20px 60px", maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {BLOG_POSTS.map((post) => (
            <Link key={post.slug} to={`/blog/${post.slug}`} style={{ textDecoration: "none", display: "block", background: "rgba(255,255,255,0.02)", border: "1px solid #1a1a1a", padding: mob ? "20px 16px" : "32px 28px", transition: "all 0.3s", borderLeft: `3px solid ${AC}` }}>
              ...
              <div style={{ fontSize: mob ? 17 : 20, fontWeight: 800, color: "#fff", letterSpacing: 0.5, marginBottom: 10, lineHeight: 1.3 }}>{lang === "ru" ? post.titleRu : post.titleEn}</div>
