    ...
    <div style={{ overflowX: "hidden" }}>
      ...
      <section ref={setRef("hero")} data-section="hero" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", position: "relative", padding: mob ? "100px 16px 40px" : "120px 20px 40px", textAlign: "center", background: "radial-gradient(ellipse at 50% 30%,rgba(0,240,255,0.06) 0%,transparent 60%)" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 700, width: "100%" }}>
          ...
          <button onClick={() => scrollTo("cta")} style={{ marginTop: mob ? 32 : 44, padding: mob ? "12px 28px" : "14px 40px", background: "#00f0ff", border: "none", color: "#08080c", fontSize: mob ? 11 : 12, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.cta.btn1}</button>
        </div>
      </section>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {t.how.steps.map((step, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: mob ? "40px 1fr" : "56px 1fr", alignItems: "start", padding: mob ? "16px 12px" : "24px", background: C.bgCard, borderLeft: "3px solid #00f0ff" }}>
              <div style={{ fontSize: mob ? 18 : 24, fontWeight: 900, color: "#00f0ff", fontFamily: "'Xolonium','Tektur',monospace", opacity: 0.4 }}>{step.num}</div>
              <div>
          ...
          {t.players.list.map((p) => {
            ...
            return (
              <div key={p.rank} style={{ display: "grid", gridTemplateColumns: mob ? "36px 1fr 60px" : "56px 1fr 100px 1fr", padding: mob ? "12px 12px" : "16px 24px", background: isTop3 ? `${rc}08` : C.bgCard, borderTop: `1px solid ${C.borderLight}`, alignItems: "center" }}>
                <div style={{ fontFamily: "'Xolonium','Tektur',monospace", fontSize: isTop3 ? (mob ? 14 : 16) : 13, fontWeight: isTop3 ? 900 : 600, color: rc ?? C.secondary }}>{p.rank}</div>
      ...
      <section ref={setRef("cta")} data-section="cta" style={{ padding: pad, maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: mob ? 12 : 16, justifyContent: "center", flexWrap: "wrap" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <button style={{ padding: mob ? "12px 28px" : "14px 40px", background: "#00f0ff", border: "none", color: "#08080c", fontSize: mob ? 11 : 12, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.cta.btn1}</button>
            <span style={{ fontFamily: BODY_FONT, fontSize: 10, color: C.muted, letterSpacing: 2 }}>{t.cta.btn1sub}</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <button style={{ padding: mob ? "12px 28px" : "14px 40px", background: "transparent", border: `1px solid ${C.subtle}`, color: C.body, fontSize: mob ? 11 : 12, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.cta.btn2}</button>
