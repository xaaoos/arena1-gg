...
const Verified: FC = () => {
  ...
  return (
    <div>
      <ScanLine />
      <nav style={{ position: "fixed", top: 48, left: 0, right: 0, zIndex: 99, background: "rgba(8,8,12,0.92)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(0,240,255,0.12)", display: "flex", justifyContent: "center", overflowX: "auto" }}>
        {NAV_IDS.map((id, i) => (
          <button key={id} onClick={() => scrollTo(id)} style={{ background: "none", border: "none", cursor: "pointer", color: active === id ? "#00f0ff" : "#555", fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", padding: "12px 12px", whiteSpace: "nowrap", borderBottom: active === id ? "2px solid #00f0ff" : "2px solid transparent", transition: "all 0.3s", fontFamily: "'Orbitron',sans-serif" }}>{t.nav[i]}</button>
        ))}
      </nav>

      {/* Hero */}
      <section ref={setRef("hero")} data-section="hero" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", position: "relative", padding: "120px 20px 40px", textAlign: "center", background: "radial-gradient(ellipse at 50% 30%,rgba(0,240,255,0.06) 0%,transparent 60%)" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 700 }}>
          <div style={{ marginBottom: 40, display: "flex", justifyContent: "center" }}><Icon type="shield" color="#00f0ff" size={48} /></div>
          <div style={{ fontSize: 11, letterSpacing: 6, color: "#00f0ff", marginBottom: 20, fontWeight: 600 }}>{t.hero.badge}</div>
          <h1 style={{ fontSize: "clamp(32px,6vw,64px)", fontWeight: 900, margin: 0, lineHeight: 1, color: "#fff" }}>{t.hero.t1}<br /><span style={{ color: "#00f0ff" }}>{t.hero.t2}</span></h1>
          <div style={{ fontFamily: BODY_FONT, fontSize: 15, color: "#888", marginTop: 28, lineHeight: 1.8, maxWidth: 560, margin: "28px auto 0" }}>{t.hero.text}</div>
          <button onClick={() => scrollTo("cta")} style={{ marginTop: 44, padding: "14px 40px", background: "#00f0ff", border: "none", color: "#08080c", fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Orbitron',monospace" }}>{t.cta.btn1}</button>
        </div>
      </section>

      {/* Problem */}
      <section ref={setRef("problem")} data-section="problem" style={{ padding: "120px 20px", maxWidth: 860, margin: "0 auto" }}>
        <SL num={t.problem.num} text={t.problem.label} color="#00f0ff" />
        <ST>{t.problem.t1}<br /><span style={{ color: "#ff3e3e" }}>{t.problem.t2}</span></ST>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 2, marginBottom: 32 }}>
          {t.problem.items.map((item, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,0.02)", padding: "28px 24px", borderTop: `3px solid ${PROB_C[i]}` }}>
        ...
        <div style={{ padding: "20px 28px", background: "rgba(255,62,62,0.04)", borderLeft: "3px solid #ff3e3e" }}>
          <div style={{ fontFamily: BODY_FONT, fontSize: 14, color: "#ff3e3e", lineHeight: 1.6, fontWeight: 500 }}>{t.problem.result}</div>
        </div>
      </section>

      {/* How */}
      <section ref={setRef("how")} data-section="how" style={{ padding: "120px 20px", maxWidth: 800, margin: "0 auto" }}>
        <SL num={t.how.num} text={t.how.label} color="#00f0ff" />
        <ST>{t.how.t1}<br /><span style={{ color: "#00f0ff" }}>{t.how.t2}</span></ST>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {t.how.steps.map((step, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "56px 1fr", alignItems: "start", padding: 24, background: "rgba(255,255,255,0.02)", borderLeft: "3px solid #00f0ff" }}>
              <div style={{ fontSize: 24, fontWeight: 900, color: "#00f0ff", fontFamily: "'Orbitron',monospace", opacity: 0.4 }}>{step.num}</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#fff", letterSpacing: 0.5, marginBottom: 8 }}>{step.title}</div>
      ...
      {/* Who */}
      <section ref={setRef("who")} data-section="who" style={{ padding: "120px 20px", maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(250px,1fr))", gap: 2 }}>
          {t.who.items.map((item, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,0.02)", padding: "32px 24px", borderTop: `3px solid ${WHO_C[i]}` }}>
      ...
      <section ref={setRef("why")} data-section="why" style={{ padding: "120px 20px", maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 2 }}>
          {t.why.items.map((item, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,0.02)", padding: "28px 24px", borderLeft: `3px solid ${WHY_C[i]}` }}>
      ...
      {/* CTA */}
      <section ref={setRef("cta")} data-section="cta" style={{ padding: "120px 20px", maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
        <SL num={t.cta.num} text={t.cta.label} color="#00f0ff" />
        <ST>{t.cta.t1}<br /><span style={{ color: "#00f0ff" }}>{t.cta.t2}</span></ST>
        <p style={{ fontFamily: BODY_FONT, fontSize: 13, color: "#666", marginBottom: 48, lineHeight: 1.6 }}>{t.cta.desc}</p>
        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <button style={{ padding: "14px 40px", background: "#00f0ff", border: "none", color: "#08080c", fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Orbitron',monospace" }}>{t.cta.btn1}</button>
            <span style={{ fontFamily: BODY_FONT, fontSize: 10, color: "#555", letterSpacing: 2 }}>{t.cta.btn1sub}</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <button style={{ padding: "14px 40px", background: "transparent", border: "1px solid #444", color: "#888", fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Orbitron',monospace" }}>{t.cta.btn2}</button>
            <span style={{ fontFamily: BODY_FONT, fontSize: 10, color: "#555", letterSpacing: 2 }}>{t.cta.btn2sub}</span>
          </div>
        </div>
      </section>

      <footer style={{ padding: "60px 20px", textAlign: "center", borderTop: "1px solid rgba(0,240,255,0.08)" }}>
        <div style={{ fontFamily: BODY_FONT, fontSize: 14, color: "#666", fontStyle: "italic", lineHeight: 1.8, marginBottom: 24 }}>{t.fq1}<br /><span style={{ color: "#00f0ff", fontWeight: 500 }}>{t.fq2}</span></div>
