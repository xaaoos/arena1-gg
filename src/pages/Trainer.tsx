      ...
      <div style={{ maxWidth: 520, width: "100%" }}>
        ...
        {screen === "start" && (
          <div style={{ textAlign: "center" }}>
            ...
            <a href="https://t.me/quaketimingbot" target="_blank" rel="noopener noreferrer" style={{ display: "inline-block", padding: mob ? "10px 20px" : "12px 32px", background: "#fbbf24", border: "none", color: "#08080c", fontSize: mob ? 10 : 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace", textDecoration: "none", marginBottom: mob ? 28 : 40 }}>{t.tgBtn}</a>
            <div style={{ display: "flex", gap: 2, marginBottom: 32 }}>
              {([{ m: 25, l: t.modeRA, s: t.modeRASub }, { m: 35, l: t.modeMH, s: t.modeMHSub }, { m: "mix", l: t.modeMix, s: t.modeMixSub }] as const).map((o) => (
                <button key={String(o.m)} onClick={() => setMode(o.m)} style={{ flex: 1, padding: mob ? "14px 6px" : "20px 12px", background: mode === o.m ? "rgba(255,62,62,0.08)" : C.bgCard, border: "none", borderTop: mode === o.m ? "3px solid #ff3e3e" : `3px solid ${C.border}`, cursor: "pointer", textAlign: "center" }}>
                  <div style={{ fontSize: mob ? 12 : 14, fontWeight: 700, color: mode === o.m ? "#ff3e3e" : C.body, fontFamily: "'Xolonium','Tektur',monospace" }}>{o.l}</div>
                  <div style={{ fontFamily: BODY_FONT, fontSize: mob ? 10 : 11, color: C.muted, marginTop: 6 }}>{o.s}</div>
                </button>
              ))}
            </div>
            <button onClick={startTraining} style={{ width: "100%", padding: 16, background: "#ff3e3e", border: "none", color: "#fff", fontSize: 13, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.start}</button>
        ...
        {screen === "play" && cards.length > 0 && (
          <div>
            ...
            <div style={{ background: checked ? (isCorrect ? "rgba(74,222,128,0.06)" : "rgba(248,113,113,0.06)") : C.bgCard, border: checked ? `2px solid ${isCorrect ? "#4ade80" : "#f87171"}` : `2px solid ${C.border}`, padding: mob ? "28px 16px" : "40px 24px", textAlign: "center", marginBottom: 20, transition: "all 0.3s" }}>
              ...
              <div style={{ fontSize: "clamp(48px,12vw,72px)", fontWeight: 900, color: "#ff3e3e", fontFamily: "'Xolonium','Tektur',monospace", lineHeight: 1 }}>{fmt(cards[idx].pickup)}</div>
              <div style={{ fontFamily: BODY_FONT, fontSize: 13, color: C.body, marginTop: 16 }}>{t.qPre} {fmt(cards[idx].pickup)}. {t.qSuf}</div>
              {checked && <div style={{ fontSize: 36, fontWeight: 900, fontFamily: "'Xolonium','Tektur',monospace", marginTop: 20, color: isCorrect ? "#4ade80" : "#f87171" }}>{fmt(cards[idx].respawn)}</div>}
            </div>
            {!checked ? (
              <div style={{ display: "flex", gap: 8 }}>
                <input ref={inputRef} type="text" inputMode="numeric" maxLength={2} value={input} onChange={(e) => { if (/^\d{0,2}$/.test(e.target.value)) setInput(e.target.value); }} placeholder={t.ph} style={{ flex: 1, padding: 18, fontSize: 28, textAlign: "center", fontWeight: 700, border: `2px solid ${C.inputBorder}`, background: C.inputBg, color: C.heading, outline: "none", fontFamily: "'Xolonium','Tektur',monospace" }} />
                <button onClick={checkAnswer} style={{ padding: "18px 32px", background: "#ff3e3e", border: "none", color: "#fff", fontSize: 12, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.check}</button>
              </div>
            ) : (
              <button onClick={nextCard} style={{ width: "100%", padding: 18, background: C.bgCard, border: `1px solid ${C.border}`, color: C.heading, fontSize: 13, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.next}</button>
            )}
          </div>
        )}
        ...
        {screen === "results" && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: "rgba(255,62,62,0.1)", marginBottom: 32 }}>
              {[{ v: `${accuracy}%`, c: "#4ade80", l: t.res.accuracy }, { v: fmtT(elapsed), c: "#fbbf24", l: t.res.totalTime }, { v: `${(elapsed / 60000).toFixed(1)}s`, c: C.body, l: t.res.avgTime }].map((s, i) => (
                <div key={i} style={{ background: C.bg, padding: "24px 16px", transition: "background 0.3s" }}>
                  <div style={{ fontSize: 28, fontWeight: 900, color: s.c, fontFamily: "'Xolonium','Tektur',monospace" }}>{s.v}</div>
              ...
              <div style={{ margin: "0 0 32px", padding: "28px 24px", background: C.bgCard, border: `1px solid ${C.border}` }}>
                ...
                {!emailSent ? (
                  <div style={{ display: "flex", gap: 8, maxWidth: 400, margin: "0 auto" }}>
                    ...
                    <button onClick={submitEmail} style={{ padding: "14px 24px", background: "#ff3e3e", border: "none", color: "#fff", fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace", whiteSpace: "nowrap" }}>{t.res.emailBtn}</button>
                  </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={startTraining} style={{ flex: 1, padding: 16, background: "#ff3e3e", border: "none", color: "#fff", fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.res.retry}</button>
              <button onClick={() => setScreen("start")} style={{ flex: 1, padding: 16, background: "transparent", border: `1px solid ${C.subtle}`, color: C.body, fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Xolonium','Tektur',monospace" }}>{t.res.back}</button>
            </div>
          </div>
        )}
      </div>
