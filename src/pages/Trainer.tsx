      ...
      <div style={{ maxWidth: 520, width: "100%" }}>
        ...
        {screen === "start" && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontFamily: BODY_FONT, fontSize: 11, color: "#444", marginTop: 12 }}>{t.startDesc}</div>
          </div>
        )}

        {screen === "play" && cards.length > 0 && (
          <div>
            <div style={{ height: 3, background: "#1a1a1a", marginBottom: 20, overflow: "hidden" }}><div style={{ height: "100%", width: `${(idx / 60) * 100}%`, background: "linear-gradient(90deg,#ff3e3e,#ff8800)", transition: "width 0.3s" }} /></div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", marginBottom: 16 }}>
              {[{ v: correct, c: "#4ade80", l: t.correct }, { v: wrong, c: "#f87171", l: t.wrong }, { v: fmtT(elapsed), c: "#fbbf24", l: t.time }, { v: `${idx + 1}/60`, c: "#888", l: t.card }].map((s, i) => (
            ...
            {!checked ? (
              <div style={{ display: "flex", gap: 8 }}>
                <input ref={inputRef} type="text" inputMode="numeric" maxLength={2} value={input} onChange={(e) => { if (/^\d{0,2}$/.test(e.target.value)) setInput(e.target.value); }} placeholder={t.ph} style={{ flex: 1, padding: 18, fontSize: 28, textAlign: "center", fontWeight: 700, border: "2px solid #333", background: "rgba(255,255,255,0.03)", color: "#fff", outline: "none", fontFamily: "'Orbitron',monospace" }} />
              ...
              <button onClick={nextCard} style={{ width: "100%", padding: 18, background: "rgba(255,255,255,0.06)", border: "1px solid #333", color: "#fff", fontSize: 13, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Orbitron',monospace" }}>{t.next}</button>
            )}
          </div>
        )}
        ...
        {screen === "results" && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontFamily: BODY_FONT, fontSize: 13, color: "#888", marginBottom: 32, lineHeight: 1.6 }}>{gd.f}</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: "rgba(255,62,62,0.1)", marginBottom: 32 }}>
              {[{ v: `${accuracy}%`, c: "#4ade80", l: t.res.accuracy }, { v: fmtT(elapsed), c: "#fbbf24", l: t.res.totalTime }, { v: `${(elapsed / 60000).toFixed(1)}s`, c: "#888", l: t.res.avgTime }].map((s, i) => (
                <div key={i} style={{ background: "#08080c", padding: "24px 16px" }}>
                  <div style={{ fontSize: 28, fontWeight: 900, color: s.c, fontFamily: "'Orbitron',monospace" }}>{s.v}</div>
                  <div style={{ fontSize: 9, letterSpacing: 2, color: "#555", marginTop: 6, textTransform: "uppercase" }}>{s.l}</div>
                </div>
              ))}
            </div>

            {errors.length > 0 && !unlocked && (
              <div style={{ margin: "0 0 32px", padding: "28px 24px", background: "rgba(255,255,255,0.02)", border: "1px solid #222" }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#fff", marginBottom: 6 }}>{t.res.emailGate}</div>
                <div style={{ fontFamily: BODY_FONT, fontSize: 12, color: "#666", marginBottom: 24, lineHeight: 1.6 }}>{t.res.emailDesc}</div>
                {!emailSent ? (
                  <div style={{ display: "flex", gap: 8, maxWidth: 400, margin: "0 auto" }}>
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t.res.emailPh}
                      onKeyDown={(e) => { if (e.key === "Enter") submitEmail(); }}
                      style={{ flex: 1, padding: "14px 16px", fontSize: 14, border: "2px solid #333", background: "rgba(255,255,255,0.03)", color: "#fff", outline: "none", fontFamily: BODY_FONT }} />
                ...
                <div style={{ display: "flex", gap: 2, flexDirection: "column" }}>
                  {errors.map((e, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", padding: "12px 16px", background: "rgba(248,113,113,0.04)", borderLeft: "3px solid #f87171" }}>
                      <div><div style={{ fontSize: 9, color: "#555", letterSpacing: 1, marginBottom: 2 }}>{t.res.pickup}</div><div style={{ fontFamily: BODY_FONT, fontSize: 14, color: "#fff", fontWeight: 700 }}>{fmt(e.pickup)}</div></div>
                      <div><div style={{ fontSize: 9, color: "#555", letterSpacing: 1, marginBottom: 2 }}>{t.res.you}</div><div style={{ fontFamily: BODY_FONT, fontSize: 14, color: "#f87171", fontWeight: 700 }}>{e.yours !== null ? fmt(e.yours) : "—"}</div></div>
                      <div><div style={{ fontSize: 9, color: "#555", letterSpacing: 1, marginBottom: 2 }}>{t.res.cor}</div><div style={{ fontFamily: BODY_FONT, fontSize: 14, color: "#4ade80", fontWeight: 700 }}>{fmt(e.correct)}</div></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => setScreen("start")} style={{ flex: 1, padding: 16, background: "transparent", border: "1px solid #444", color: "#888", fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", fontFamily: "'Orbitron',monospace" }}>{t.res.back}</button>
            </div>
          </div>
        )}
      </div>
