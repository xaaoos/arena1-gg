...
const BlogPost: FC = () => {
  ...
  return (
    <div>
      <ScanLine />
      <article style={{ maxWidth: 720, margin: "0 auto", padding: "120px 20px 60px" }}>
        {/* Back link */}
        <Link to="/blog" style={{ fontFamily: BODY_FONT, fontSize: 12, color: AC, textDecoration: "none", letterSpacing: 1, display: "inline-block", marginBottom: 40 }}>&larr; {lang === "ru" ? "Блог" : "Blog"}</Link>
        ...
        <h1 style={{ fontSize: "clamp(28px,5vw,44px)", fontWeight: 900, color: "#fff", lineHeight: 1.15, margin: "0 0 48px", letterSpacing: -0.5 }}>{title}</h1>

        {/* Content */}
        <div>
          {blocks.map((block, i) => {
            if (block.type === "h2") {
              return <h2 key={i} style={{ fontSize: 22, fontWeight: 800, color: "#fff", margin: "48px 0 16px", letterSpacing: 0.3, lineHeight: 1.2 }}>{block.text}</h2>;
            }
            const parts = renderInline(block.text);
            return (
              <p key={i} style={{ fontFamily: BODY_FONT, fontSize: 15, color: "#aaa", lineHeight: 1.85, margin: "0 0 18px" }}>
                {parts.map((part, j) =>
        ...
        {/* Footer nav */}
        <div style={{ marginTop: 64, paddingTop: 32, borderTop: "1px solid #1a1a1a", display: "flex", justifyContent: "space-between" }}>
