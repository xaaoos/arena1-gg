  ...
  return (
    <div style={{ overflowX: "hidden" }}>
      ...
      <article style={{ maxWidth: 720, margin: "0 auto", padding: mob ? "80px 16px 40px" : "120px 20px 60px" }}>
        ...
        <h1 style={{ fontSize: "clamp(24px,5vw,44px)", fontWeight: 900, color: C.heading, lineHeight: 1.15, margin: mob ? "0 0 32px" : "0 0 48px", letterSpacing: -0.5 }}>{title}</h1>

        <div>
          {blocks.map((block, i) => {
            if (block.type === "h2") {
              return <h2 key={i} style={{ fontSize: mob ? 18 : 22, fontWeight: 800, color: C.heading, margin: mob ? "36px 0 12px" : "48px 0 16px", letterSpacing: 0.3, lineHeight: 1.2 }}>{block.text}</h2>;
            }
            const parts = renderInline(block.text);
            return (
              <p key={i} style={{ fontFamily: BODY_FONT, fontSize: mob ? 14 : 15, color: C.link, lineHeight: 1.85, margin: "0 0 18px" }}>
                {parts.map((part, j) =>
                  typeof part === "string" ? <span key={j}>{part}</span> : <strong key={j} style={{ color: C.heading, fontWeight: 700 }}>{part.bold}</strong>
                )}
              </p>
            );
          })}
        ...
        <div style={{ marginTop: mob ? 40 : 64, paddingTop: 32, borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between" }}>
